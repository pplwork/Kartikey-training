...
const DiscoverPeopleList = () => {
  ...
  useEffect(() => {
    (async () => {
      let docs = (await firestore().collection("users").get()).docs;
      let users = docs.map((doc) => ({ uid: doc.id, data: doc.data() }));
      users = users.filter(
        (ele) =>
          !user.Following.includes(ele.uid) && ele.uid != auth().currentUser.uid
      );
      // get image, mutuals,name
      let people = await Promise.all(
        users.map(async (ele) => {
          let uri = await storage().refFromURL(ele.data.Photo).getDownloadURL();
          let mutual = [];
